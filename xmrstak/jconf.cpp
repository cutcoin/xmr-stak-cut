...
const std::string jconf::GetCurrency()
{
	...
	if(
			...
			// if cutcoin is disabled at compile time, enable error message if selected currency is `cutcoin`
			!xmrstak::strcmp_i(currency, "cutcoin")
#else
			true
#endif
			&&
...
{
	if(xmrstak::strcmp_i(GetCurrency(), "cutcoin"))
	{
		return true;
	}
	else
