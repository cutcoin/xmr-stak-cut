...
void help()
{
	...
	cout<<"  --currency NAME       currency to mine: cutcoin or aeon"<<endl;
#endif
#ifndef CONF_NO_CPU
...
std::string get_multipool_entry(bool& final)
{
	...
	std::string pool;
	if(xmrstak::params::inst().currency == "cutcoin")
		std::cout<<"- Pool address: e.g. pool.cutcoin.org:5555"<<std::endl;
	else
...
void do_guided_config()
{
	...
	if(currency.empty())
	{
		...
		tmp = "aeon";
#endif
		while(tmp != "cutcoin" && tmp != "aeon")
		{
			std::cout<<"- Currency: 'cutcoin' or 'aeon'"<<std::endl;
			std::cin >> tmp;
			std::transform(tmp.begin(), tmp.end(), tmp.begin(), ::tolower);
		} 
		currency = tmp;
...
int main(int argc, char *argv[])
{
	...
	if(::jconf::inst()->IsCurrencyMonero())
		printer::inst()->print_msg(L0,"Start mining: CUTCOIN");
	else
